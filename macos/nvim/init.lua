...
require("lazy").setup({
	...
	-- Better syntax highlighting & much more
	{
		...
		build = ":TSUpdate",
		config = function()
			local filetypes = { "c", "lua", "typescript", "vim", "vimdoc", "query", "markdown", "markdown_inline" }
...
local function get_python_path(workspace)
	-- If in a Poetry project, use its virtualenv python
	if vim.fn.filereadable(workspace .. "/poetry.lock") == 1 then
		local venv = vim.fn.trim(vim.fn.system("cd " .. vim.fn.shellescape(workspace) .. " && poetry env info -e 2>/dev/null"))
		if vim.v.shell_error == 0 and venv ~= "" then
			return venv
		end
	end
	-- Fallback: .venv in project
	local dot_venv = workspace .. "/.venv/bin/python"
